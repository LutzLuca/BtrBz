...
public class ScreenInventoryTracker {
    ...
    private void handleSlotUpdate(ClientboundContainerSetSlotPacket packet) {
        ...
        boolean allSlotsReceived = this.currInv.updateSlot(slot, packet.getItem());
        if (!this.currInv.fullyLoaded && allSlotsReceived) {
            this.loaded();
        }
    }
    ...
    public static class Inventory {
        ...
        private final BitSet receivedSlots = new BitSet();
        ...
        boolean updateSlot(int slot, ItemStack itemStack) {
            if (itemStack.isEmpty()) {
                this.items.remove(slot);
            } else {
                this.items.put(slot, itemStack);
            }
            this.receivedSlots.set(slot);
            return this.receivedSlots.cardinality() == this.slotCount;
        }
