...
public final class BazaarOrdersWidgetSettings {
    ...
    private static void populate(
        ...
        WidgetSettingsPanel.enumeration(panel, "Display mode", binding, c -> c.mode, (c, v) -> c.mode = v,
        ...
        WidgetSettingsPanel.integer(panel, "Widget width", binding, c -> c.contentWidth,
            (c, v) -> c.contentWidth = v, 180, 320,
            "Controls horizontal space without changing the widget's text or icon scale.");

        if (binding.current().mode != BazaarOrdersWidgetConfig.HudMode.Detailed) {
            return;
        }

        WidgetSettingsPanel.integer(
            panel,
            "Visible orders",
            binding,
            c -> c.visibleOrders,
        ...
        WidgetSettingsPanel.bool(panel, "Abbreviate Enchanted", binding, c -> c.abbreviateEnchanted,
            (c, v) -> c.abbreviateEnchanted = v,
            "Shortens names beginning with Enchanted when horizontal space is limited.");

        WidgetSettingsPanel.bool(panel, "Show queue information", binding, c -> c.showQueue, (c, v) -> c.showQueue = v,
            "Shows orders and items ahead as [orders/items] when the data is available.");

        WidgetSettingsPanel.bool(panel, "Show undercut gap", binding, c -> c.showUndercutGap,
            (c, v) -> c.showUndercutGap = v,
